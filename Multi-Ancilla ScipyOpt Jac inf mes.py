...
def sciopt(single_input):
    ...
    def jac(angles, matrix, nnq, nnanc, circuit_depth, bitmap): # Jacobian for SLSQP
        return na.cfgrad_vec_inf(angles, matrix, nnanc,nreg, bitmap, [x for x in range(len(angles))])
    ...
    options_ = {'maxiter': 1000, 'ftol': 2e-2}
    cray1 = time.time()
    opt = minimize(vqee, initvec,args=(amat, nq, nanc, dp, bitmap),method=copt,jac=jac, options=options_)
    
    cflist.append(opt.fun)
    optvec.append(opt.x)
    time_taken = time.time()- cray1
    
    file1 = "CF-"+copt+"jv1-nc"+str(len(amat))+"-"+str(matseed)+" - na" +str(nanc)+ "dp"+str(dp)+"minf.npy"                            
    file2 = "Optv-"+copt+"jv1-nc"+str(len(amat))+"-"+str(matseed)+" - na" +str(nanc)+ "dp"+str(dp)+"minf.npy"
    ...
    curr_process_file = "callback_savefile"+str(seed)+"-"+str(matseed)+"-"+str(time.time()%1)[-6:]+".npy"          
    ...
    os.remove(curr_process_file)                 # Remove place in queue so others can go
...
if __name__ == '__main__':
    print()
    print("Starting script.... Quantum circuit, infinite measurements, scipy optimizer with jacobian")
    print()
    na.loadstat()
